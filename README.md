...
全屏使用 macOS 原生 Space 切换，可通过应用全屏按钮或 `Control + Command + F` 进入，按 `Esc` 退出。退出后恢复原窗口位置和尺寸；外接显示器移除时将恢复区域限制在仍可用的屏幕内。全屏动画中不会重复调整窗口大小。系统隐藏、最小化由 macOS 管理，应用不会强制弹回前台。

兼容范围与验证情况：

- 已在 Apple Silicon Mac 上完成 `.app` 构建和实际启动，启动状态为 `ready`，本地页面返回 HTTP 200。
- 图形启动参数、原生全屏异步切换、窗口恢复、本地音乐持久化和启动流程具备回归检查。具体结果见 PR 验证记录。
- 本轮全屏修复已通过自动化检查与 `.app` 构建，原生全屏/Dock/多屏、播放、平台登录、菜单/托盘和视觉效果仍需实机交互验收，不能以构建成功代替验收。DMG 生成和 Intel Mac 未验证。
